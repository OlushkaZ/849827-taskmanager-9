...
export class Filter {
  ...
  <label for="filter__${filterItem.title}" class="filter__label">
    ${filterItem.title} <span class="filter__${filterItem.title}-count">${getFilterCount(filterItem.title, this._tasks)}</span></label
  >`).join(``)}
...
const getFilterCount = (title, tasks)=> {
  switch (title) {
    case `all`:
      return tasks.length;
    case `overdue`:
      return tasks.filter(({dueDate})=>isOverdue(dueDate)).length;
    case `today`:
      return tasks.filter(({dueDate})=>isToday(dueDate)).length;
    case `favorites`:
